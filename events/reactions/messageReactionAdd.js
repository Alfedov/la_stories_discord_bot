...
.setDescription(`${message.client.emoji.fail} Притормози там, у тебя перезарядка\n\n**Название роли:** ${rrRole.name}\n**Название гильдии:** ${guildName}`)
.setColor(message.client.color.red)

let addEmbed = new MessageEmbed()
.setAuthor('Роль добавлена', `${message.client.domain}/logo.png` , `${message.url}` )
.setDescription(`Вы получили **${rrRole.name}** С помощью реакции в ${guildName}`)
.setColor(message.client.color.green)

let remEmbed = new MessageEmbed()
.setAuthor('Роль удалена', `${message.client.domain}/logo.png` , `${message.url}` )
.setDescription(`Вы удалили роль **${rrRole.name}** С помощью реакции в ${guildName}`)
.setColor(message.client.color.green)

let errorReaction = new MessageEmbed()
.setAuthor('Ошибка роли реакции', `${message.client.domain}/logo.png` , `${message.url}` )
.setDescription(`${message.client.emoji.fail} Не удалось добавить роль, так как у меня отсутствует разрешение на управление ролями.\n\nПожалуйста, сообщите администратору.`)
.setColor(message.client.color.green)
...
let arraylength = array.length
      ...
      if(arraylength > ticketlimit || arraylength == ticketlimit) {
        ...
        message.channel.send(new discord.MessageEmbed().setColor(message.client.color.red).setDescription(`You already have ${arraylength} open tickets, as the current guild's ticket limit is ${ticketlimit} `).setAuthor(user.tag, user.displayAvatarURL())).then(m => m.delete({timeout: 5000}));
